...
output "ssh_command" {
  ...
  value       = "ssh -i ${replace(var.ssh_public_key_path, "~", "$HOME")} ${var.admin_username}@${data.azurerm_public_ip.public_ip.fqdn}"
}

output "aap_console_url" {
  description = "The URL to access the Ansible Automation Platform Web Console."
  value       = "https://${data.azurerm_public_ip.public_ip.fqdn}"
}
